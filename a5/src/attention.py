...
class SynthesizerAttention(nn.Module):
    ...
    def forward(self, x, layer_past=None):
        ...
        att = F.relu(self.w1(x).view(B, T, self.n_head, C // self.n_head).transpose(1, 2)) # (B, h, T, d/h)
        att = torch.matmul(att, self.w2) + self.b2
        
        att = att[:, :, :T, :T] #what the hell happened here, without this dimensions don't match, test-time
