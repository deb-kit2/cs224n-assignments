...
if args.variant == 'vanilla':
    ...
    gptModel = model.GPT(mconf).to(device)
...
if args.function == 'pretrain':
    ...
    with open(args.pretrain_corpus_path) as openfile :
        text = openfile.read().strip()
        
    pretrainData = dataset.CharCorruptionDataset(text, block_size)
    ...
    trainer = trainer.Trainer(gptModel, pretrainData, None, tconf)
    trainer.train()
    ...
    with open(args.finetune_corpus_path) as openfile :
        text = openfile.read()
    finetuneData = dataset.NameDataset(pretrain_dataset, text)

    tconf = trainer.TrainerConfig(
                max_epochs = EPOCHS, batch_size = 256, learning_rate = 6e-4, lr_decay = True,
                warmup_tokens = 512*20, final_tokens = 200*len(pretrain_dataset)*block_size,
                num_workers = 4, ckpt_path = args.writing_params_path
            )

    trainer = trainer.Trainer(gptModel, finetuneData, None, tconf)
    trainer.train()
    # raise NotImplementedError
    ...
    with open(args.outputs_path, 'w') as fout:
        ...
        for line in tqdm(open(args.eval_corpus_path)):
            ...
            pred = utils.sample(gptModel, x, 32, sample=False)[0]
            completion = ''.join([pretrain_dataset.itos[int(i)] for i in pred])
            pred = completion.split(u"\u2047")[1]
