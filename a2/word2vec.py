...
def negSamplingLossAndGradient(
    ...
    s = sigmoid(y_hat[outsideWordIdx])
    z = sigmoid(-y_hat[negSampleWordIndices])

    loss = -np.log(s) - np.sum(np.log(z))

    gradCenterVec += (s-1)*outsideVectors[outsideWordIdx] + (1-z).dot(u_k) 
    gradOutsideVecs[outsideWordIdx] += (s-1)*centerWordVec
    
    for i,ind in enumerate(negSampleWordIndices) :
        gradOutsideVecs[ind] += (1-z[i])*centerWordVec
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords :
        ...
        l, gcenter, gout = word2vecLossAndGradient(vc, o, outsideVectors, dataset, )
        
        loss += l
        gradCenterVecs[c] += gcenter
