...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.decoder = nn.LSTMCell(embed_size + self.hidden_size, self.hidden_size, bias = True) # check
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens = enc_hiddens.swapaxes(0, 1) # (src_len, b, h*2) -> (b, src_len, h*2)
        
        last_hidden = torch.cat((last_hidden[0], last_hidden[1]), dim = -1) # (2, b, h) -> (b, 2h)
        init_decoder_hidden = self.h_projection(last_hidden)
        
        last_cell = torch.cat((last_cell[0], last_cell[1]), dim = -1) # (2, b, h) -> (b, 2h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.bmm(enc_hiddens_proj, torch.unsqueeze(dec_hidden, dim = 2)) # (b, src_len, 1) = (b, src_len, h) . (b, h, 1)
        ...
        enc_hiddens = enc_hiddens.swapaxes(1, 2)
        a_t = torch.bmm(enc_hiddens, torch.unsqueeze(alpha_t, dim = 2))
        a_t = torch.squeeze(a_t, dim = 2) # (b, 2h)
